...
const Movies = () => {
  ...
  const [cat, setCat] = useState("now_playing"); // Default category
  ...
  const {
    ...
    error,
    loading: fetching,
  } = useFetch(`movie/${cat}?page=${page}`); // Fetch data based on filters
    ...
    <div className="md:w-[80vw] w-full">
      ...
      <div className="px-5 flex justify-between mt-5">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">Movies <span className="text-xs text-zinc-500">({cat.split('_').join(' ')})</span></h1>
        </div>
      ...
      {cardsData.length > 0 ? (
        ...
        >
          <VCards title='movie' cardsData={cardsData} /> {/* Display fetched cards */}
        </InfiniteScroll>
      ) : fetching ? (
