...
const Trending = () => {
  ...
  const [cat, setCat] = useState("all"); // Default category
  ...
  const {
    ...
    error,
    loading: fetching,
  } = useFetch(`trending/${cat}/${duration}?page=${page}`); // Fetch data based on filters
  ...
  useEffect(() => {
    if (page === 1) {
      fetchTrendingData(); // Fetch initial data
    } else if (fetchedData !== null) {
      fetchTrendingData(); // Fetch data when page changes
    }
  ...
  useEffect(() => {
    if (fetchedData && Array.isArray(fetchedData) && fetchedData.length > 0) {
      fetchTrendingData(); // Fetch new data when the page changes
    }
    ...
    <div className="md:w-[80vw] w-full">
      ...
      <div className="px-5 flex justify-between mt-5">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">Trending <span className="text-xs text-zinc-500">({cat.split('_').join(' ')})</span></h1>
        </div>
        <div className="flex gap-4">
          ...
          <Dropdown
            title="Duration"
            options={["week", "day"]}
            func={(e) => setDuration(e.target.value)} // Update duration filter
          />
        </div>
      </div>
      ...
      {cardsData.length > 0 ? (
        ...
        >
          <VCards cardsData={cardsData} /> {/* Display fetched cards */}
        </InfiniteScroll>
      ) : fetching ? (
